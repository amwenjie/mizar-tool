...
export default function loader(source) {
    ...
    if (
        ...
        const outterReg = /\bconnect\s*\(.*?\)\s*\((.+?)\)(?:$|;)/;
        const outterMatched = outterReg.exec(source);
        ...
        if (outterMatched) {
            ...
            let emsg: string;
            let injectedReducerImport: string = "";
            if (shouldInjectReducer) {
                ...
                if (fs.existsSync(reducerPath)) {
                    ...
                    if (/export\s+default\s+function[^(]*\(/.test(reducerContent)) {
                        paramArr.splice(1, 0, "alcorInjectedReducer");
                        injectedReducerImport = `import { default as alcorInjectedReducer } from "./reducer";`;
                    } else if ((new RegExp(`export\\s+function\\s+${firstLetterLowerCaseName}Reducer\\s*\\(`)).test(reducerContent)) {
                        paramArr.splice(1, 0, "alcorInjectedReducer");
                        injectedReducerImport = `import { ${firstLetterLowerCaseName}Reducer as alcorInjectedReducer } from "./reducer";`;
                    } else {
            ...
            if (shouldInjectReducer || shouldInjectReducerName) {
                ...
                callback(null, injectedReducerImport + source.replace(outterMatched[0], str));
                return;
            }
        }
    }
